...
export default function PostPage() {
  ...
  const categoryColor = post?.category === 'LOST' ? 'bg-red-500 text-white' : 'bg-green-500 text-white';
  ...
  return (
    <main className='p-3 flex flex-col max-w-6xl mx-auto min-h-screen'>
      ...
      >
        <Button className={`${categoryColor} pill size='xl'`}>
          {post && post.category}
        </Button>
      </Link>
      <div className='flex flex-wrap justify-center gap-4 mt-3'>
        ...
        <div className='flex items-center gap-2 p-2 border rounded-lg shadow-sm'>
          <span className='text-sm'>
            Item Category: <span className='font-semibold'>{post?.itemCategory}</span>
          </span>
        </div>
      </div>
      <img
